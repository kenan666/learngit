'''
#  利用  map  方法对 TFRecord  进行操作
dataset = dataset.map.(parser)
    map  是在数据机上进行操作的常用方法之一。map（parser）表示  对数据集中的每一条数据调用参数中指定的 parser 方法，
    对每一条数据进行处理后，map 将处理后的数据包装成一个新的数据集返回。
    
例如：  dataset = dataset.map(lambda x : preprocess_for_train(x,image_size,image_size,None))

#   tf.train.batch()  tf.train.shuffle_batch()  -- 队列框架
    shuffle  batch  的独立操作方式
    
    dataset = dataset.shuffle(buffer_size)
        buffer_size 等效于tf.train.shuffle_batch  的  min_after_dequeue()参数
        
    dataset = dataset.batch(batch_size)
        batch方法的参数batch_size代表要输出的每个batch由多少条数据组成。
        
#  repeat（） 是另一种操作方法。  这个方法将数据集中的数据复制多份，其中每一份数据被称为  epoch
dataset = dataset.repeat(N)

如果数据集在repeat之前进行了shuffle操作，那么输出的每个epoch中随机shuffle的结果并不会相同。

#  concatenate()  将两个数据集顺序链接起来
   take（N） 从数据集中读取前N项数据
   skip（N） 在数据集中跳过前N项数据
   flap_map()从多项数据集中轮流读取数据
