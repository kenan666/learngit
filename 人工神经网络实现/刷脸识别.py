...
综合  样本收集  图像预处理  tf人工神经网络

python  爬虫  opencv预处理   tf神经网络   

python  爬虫部分  方法  -》image  （使用一个简单的爬虫方法）

样本 也可以用视频采集   ffmpeg（推荐使用）  opencv中通过其使用
预处理：  opencv  使用   
   1--  提取人脸   haar  adaboost    +  xml + 样本 
   2--  tf  神经网络的搭建
   
cnn  数字识别   
'''
#  load  data     准备  训练  label   -》》train
#  cnn  训练    --》  检测
...
layer2 = tf.reshape(layer1_pool,[-1,32*32*32])  #  激励层
